...
def epanechnikov_kernel(x, xi, h):
    u = (x - xi) / h
    mask = np.abs(u) <= 1  # Element-wise condition
    return np.where(mask, 0.75 * (1 - u**2), 0)  # Apply kernel formula where condition is met
...
def triangular_kernel(x, xi, h):
    ...
    return np.where(econdn, (1 - np.abs(u)), 0)   # Apply kernel formula where condition is met(int the )
...
def nadaraya_watson(x_train, y_train, x_test, kernel, h, epsilon=1e-8):
    ...
    weights = kernel(x_test[:, None], x_train_flat[None, :], h)     # Vectorized computation
...
def non_paramRegreesion(x_train, y_train, x_values, kernel_func):
    # Bandwidth parameter
    ...
    # Oversmoothed
    bandwidth = (bandwidths.max() + bandwidths.min())/2
...
non_paramRegreesion(X_train, Y_train, X_values, gaussian_kernel)
print(f"\nTriangular Kernel")
non_paramRegreesion(X_train, Y_train, X_values, triangular_kernel)
print(f"\nEpanechnikov kernel")
non_paramRegreesion(X_train, Y_train, X_values, epanechnikov_kernel)
